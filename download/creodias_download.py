...
LIS_AUG_PATH = pathlib.Path(__file__).parents[1].joinpath('data_analysis\data\Lisbon\sat')
...
def creodiasDownloadLisAug():
    ...
    for feature in json.loads(response.content.decode('utf-8'))['features']:
        ...
        ncFile = title + '/' + title + '.nc'
        with zipfile.ZipFile(filename) as z:
            with open(LIS_AUG_PATH.joinpath(title+'.nc'), 'wb') as f:
                f.write(z.read(ncFile))
                f.close()
            z.close()
        os.remove(filename)
